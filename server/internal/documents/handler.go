...
type documentStore interface {
	...
	Search(ctx context.Context, ownerID string, query string, visibility SearchVisibility, limit int, cursor *SearchCursor) ([]SearchResult, error)
...
func (h *Handler) Search(w http.ResponseWriter, r *http.Request, user auth.User) {
	...
	visibility := SearchVisibility(r.URL.Query().Get("visibility"))
	if visibility == "" {
		visibility = SearchAll
	}
	if visibility != SearchAll && visibility != SearchPrivate && visibility != SearchShared {
		writeError(w, http.StatusBadRequest, "visibility must be all, private, or shared")
		return
	}
	...
	fingerprint := searchFingerprint(query, visibility)
	cursor, err := decodeSearchCursor(r.URL.Query().Get("cursor"), fingerprint)
	if err != nil {
		writeError(w, http.StatusBadRequest, "invalid search cursor")
		return
	}
	results, err := h.store.Search(r.Context(), user.ID, query, visibility, limit+1, cursor)
	if err != nil {
...
func normalizeSearchQuery(value string) (string, error) {
	...
	if length < 1 || length > 128 {
		return "", errors.New("q must contain between 1 and 128 characters")
	}
	return normalized, nil
}

func searchFingerprint(query string, visibility SearchVisibility) string {
	digest := sha256.Sum256([]byte(query + "\x00" + string(visibility)))
...
func decodeSearchCursor(value string, fingerprint string) (*SearchCursor, error) {
	...
	return &SearchCursor{Rank: encoded.Rank, UpdatedAt: updatedAt, ID: encoded.ID}, nil
}
