...
func (s *ResendSender) SendPasswordReset(ctx context.Context, email string, resetURL string, idempotencyKey string) error {
	...
	payload := struct {
		...
		From:    s.from,
		To:      []string{email},
		Subject: "Reset your passage.md password",
		HTML:    fmt.Sprintf(`<p>We received a request to reset your passage.md password.</p><p><a href="%s">Choose a new password</a></p><p>This link expires in one hour. If you did not request this, you can ignore this email.</p>`, html.EscapeString(resetURL)),
		Text:    fmt.Sprintf("Reset your passage.md password:\n\n%s\n\nThis link expires in one hour. If you did not request this, you can ignore this email.\n", resetURL),
	}
	...
	req.Header.Set("User-Agent", "passage.md/1.0")
...
func (LoggingPasswordResetSender) SendPasswordReset(_ context.Context, email string, resetURL string, _ string) error {
	slog.Info("development password reset link", "email", email, "url", resetURL)
	return nil
}
