...
func TestMeReturnsAnonymousWithoutDatabase(t *testing.T) {
	...
	if body := rec.Body.String(); body != "{\"authenticated\":false,\"policyVersion\":\"2026-07-31\",\"publicSignupEnabled\":false}\n" {
		t.Fatalf("body = %q", body)
	}
}
...
func TestMeAdvertisesPublicSignupWhenEnabled(t *testing.T) {
	...
	if body := rec.Body.String(); body != "{\"authenticated\":false,\"policyVersion\":\"2026-07-31\",\"publicSignupEnabled\":true}\n" {
		t.Fatalf("body = %q", body)
	}
}
